...
Se a SIBS devolver `HTTP 401`, execute `php /home/acodes-406/htdocs/406.acodes.pro/scripts/diagnose-auth.php` na VPS. O comando consulta o status de uma transação inexistente e faz um POST com JSON vazio (`{}`) em `/payments`, sem dados obrigatórios para criar uma transação ou iniciar MB WAY. Mostra apenas os códigos HTTP, sem exibir credenciais. `invalid_checkout: HTTP 400` sugere que a autenticação passou e a API rejeitou o corpo inválido; `HTTP 401` indica que a autenticação ainda falha. Opcionalmente, preencha `SIBS_CLIENT_SECRET` no arquivo privado para comparar a autenticação com e sem esse cabeçalho na consulta de status. O checkout continua usando os cabeçalhos documentados pela SIBS v2 até que o resultado indique o contrário.
